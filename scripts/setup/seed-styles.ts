...
async function seedStyles(): Promise<void> {
  try {
    ...
    logger.info('Starting styles seeding...', { task: 'seed-styles' })
    ...
    for (const styleData of stylesData) {
      try {
        ...
        const existingStyle = await payload.find({
          ...
          where: {
            'title.sl': {
              equals: styleData.title.sl,
            },
          },
        })

        if (existingStyle.docs.length > 0) {
          logger.info(`Style ${styleData.title.sl} already exists, skipping...`, {
            task: 'seed-styles',
            style: styleData.title.sl,
          })
          continue
        }

        // Create the style with Slovenian locale
        const createdStyle = await payload.create({
          collection: 'styles',
          data: {
            title: styleData.title.sl,
            description: styleData.description?.sl || '',
            iconKey: styleData.iconKey,
          },
          locale: 'sl',
        })
        ...
        await payload.update({
          ...
          id: createdStyle.id,
          data: {
            title: styleData.title.en,
            description: styleData.description?.en || '',
            iconKey: styleData.iconKey,
          },
          locale: 'en',
        })

        logger.info(`Created style: ${createdStyle.title}`, {
          task: 'seed-styles',
          style: styleData.title.sl,
        })
      } catch (error) {
        logger.error(
          `Failed to create style: ${styleData.title.sl} - ${(error as Error).message}`,
          error as Error,
          {
            task: 'seed-styles',
            style: styleData.title.sl,
    ...
    logger.info('Styles seeding completed successfully', { task: 'seed-styles' })
  } catch (error) {
    logger.error('Failed to seed styles', error as Error, { task: 'seed-styles' })
    throw error
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  seedStyles()
    .then(() => {
      logger.info('Seeding completed', { task: 'seed-styles' })
      process.exit(0)
    })
    .catch((error) => {
      logger.error('Seeding failed', error as Error, { task: 'seed-styles' })
      process.exit(1)
    })
}
