...
const climatesData: ClimateData[] = [
...
async function seedClimates(): Promise<void> {
  try {
    ...
    logger.info('Starting climates seeding...', { task: 'seed-climates' })

    for (const climateData of climatesData) {
      try {
        // Check if climate already exists
        const existingClimate = await payload.find({
          collection: 'climates',
          where: {
            'title.sl': {
              equals: climateData.title.sl,
            },
          },
        })

        if (existingClimate.docs.length > 0) {
          logger.info(`Climate ${climateData.title.sl} already exists, skipping...`, {
            task: 'seed-climates',
            climate: climateData.title.sl,
          })
          continue
        }

        // Create the climate with Slovenian locale
        const createdClimate = await payload.create({
          collection: 'climates',
          data: {
            title: climateData.title.sl,
            description: climateData.description?.sl || '',
            climate: climateData.climate,
            climateTemperature: climateData.climateTemperature,
            diurnalTemperatureRange: climateData.diurnalTemperatureRange,
            climateHumidity: climateData.climateHumidity,
          },
          locale: 'sl',
        })
        ...
        await payload.update({
          collection: 'climates',
          id: createdClimate.id,
          data: {
            title: climateData.title.en,
            description: climateData.description?.en || '',
            climate: climateData.climate,
            climateTemperature: climateData.climateTemperature,
            diurnalTemperatureRange: climateData.diurnalTemperatureRange,
            climateHumidity: climateData.climateHumidity,
        ...
        logger.info(`Created climate: ${createdClimate.title}`, {
          task: 'seed-climates',
          climate: climateData.title.sl,
        })
      } catch (error) {
        logger.error(
          `Failed to create climate: ${climateData.title.sl} - ${(error as Error).message}`,
          error as Error,
          {
            task: 'seed-climates',
            climate: climateData.title.sl,
    ...
    logger.info('Climates seeding completed successfully', { task: 'seed-climates' })
  } catch (error) {
    logger.error('Failed to seed climates', error as Error, { task: 'seed-climates' })
    throw error
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  seedClimates()
    .then(() => {
      logger.info('Seeding completed', { task: 'seed-climates' })
      process.exit(0)
    })
    .catch((error) => {
      logger.error('Seeding failed', error as Error, { task: 'seed-climates' })
      process.exit(1)
    })
}
